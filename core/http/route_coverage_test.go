...
var _ = Describe("Route auth coverage", func() {
	...
	It("enforces the anonymous-access decision for every registered route", func() {
		...
		expectedPublicRoutes := map[routePattern]struct{}{
			...
			// The local shared workspace is available to each device using the
			// unauthenticated LocalAI UI.
			{method: http.MethodGet, path: "/api/chats"}: {},
			{method: http.MethodPut, path: "/api/chats"}: {},
			{method: http.MethodGet, path: "/api/maps"}:  {},
		}
