...
// Reactive model presence visualization
const brainCanvas = document.getElementById('brainCanvas');
const brainVisual = document.getElementById('brainVisual');
const brainCaption = document.getElementById('brainCaption');
const sphereMode = document.getElementById('sphereMode');
const sphereComplexity = document.getElementById('sphereComplexity');
let brainCtx = null;
let brainFrame = null;
let localBrainAnalyser = null;
let remoteBrainAnalyser = null;
let brainAudioCtx = null;
let brainLevel = 0;
let brainState = 'idle';
...
modelSelect.addEventListener('change', function() {
  ...
  updateBrainModel(opt.value, opt.dataset.llm || '');
...
function modelParameterCount(name) {
  const match = String(name || '').match(/(?:^|[-_ ])(\d+(?:\.\d+)?)\s*b(?:\b|[-_])/i);
  return match ? Number(match[1]) : 1;
}

function updateBrainModel(modelName, llmName) {
  const size = Math.max(modelParameterCount(modelName), modelParameterCount(llmName));
  const detail = Math.max(0, Math.min(1, Math.log10(size + 1) / Math.log10(71)));
  brainCanvas.dataset.nodes = String(Math.round(70 + detail * 210));
  sphereComplexity.textContent = size >= 20 ? 'High-detail presence' : size >= 7 ? 'Detailed presence' : 'Lightweight presence';
  brainCanvas.dataset.size = String(size);
}

function setBrainState(state) {
  brainState = state;
  const labels = { idle: 'Ready when you are', listening: 'Listening to you', thinking: 'Thinking', speaking: 'Speaking' };
  sphereMode.textContent = labels[state] || 'Standing by';
  brainCaption.textContent = labels[state] || 'Ready when you are';
  brainVisual.dataset.state = state;
}

function startBrainVisualizer(stream, kind) {
  if (!stream) return;
  if (!brainAudioCtx) brainAudioCtx = new AudioContext();
  const analyserNode = brainAudioCtx.createAnalyser();
  analyserNode.fftSize = 256;
  analyserNode.smoothingTimeConstant = 0.75;
  brainAudioCtx.createMediaStreamSource(stream).connect(analyserNode);
  if (kind === 'remote') remoteBrainAnalyser = analyserNode;
  else localBrainAnalyser = analyserNode;
  if (!brainFrame) drawBrain();
}

function analyserLevel(node) {
  if (!node) return 0;
...
function drawBrain() {
  ...
  brainFrame = requestAnimationFrame(drawBrain);
  const rect = brainCanvas.getBoundingClientRect();
  const dpr = window.devicePixelRatio || 1;
  const width = Math.max(1, Math.floor(rect.width * dpr));
  const height = Math.max(1, Math.floor(rect.height * dpr));
  if (brainCanvas.width !== width || brainCanvas.height !== height) { brainCanvas.width = width; brainCanvas.height = height; }
  const ctx = brainCanvas.getContext('2d');
  const w = rect.width, h = rect.height;
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, w, h);
  const incoming = Math.max(analyserLevel(localBrainAnalyser), analyserLevel(remoteBrainAnalyser));
  brainLevel += (incoming - brainLevel) * .16;
  const t = performance.now() * .001;
  const nodes = Number(brainCanvas.dataset.nodes || 90);
  const size = Number(brainCanvas.dataset.size || 1);
  const centerX = w / 2, centerY = h / 2;
  const radius = Math.min(w, h) * (.235 + brainLevel * .035);
  const points = [];
  const golden = Math.PI * (3 - Math.sqrt(5));
  for (let i = 0; i < nodes; i++) {
    const y = 1 - (i / Math.max(1, nodes - 1)) * 2;
    const ring = Math.sqrt(Math.max(0, 1 - y * y));
    const theta = golden * i + t * (.16 + brainLevel * .3);
    const depth = Math.sin(theta + t * .2) * .5 + .5;
    const scale = .72 + depth * .28;
    points.push({ x: centerX + Math.cos(theta) * ring * radius * scale, y: centerY + y * radius * scale, z: depth });
  }
  const hue = brainState === 'speaking' ? 30 : brainState === 'thinking' ? 190 : 198;
  ctx.globalCompositeOperation = 'lighter';
  for (let i = 0; i < points.length; i++) {
    const point = points[i];
    const neighbor = points[(i + Math.max(3, Math.floor(nodes / 28))) % points.length];
    const distance = Math.hypot(point.x - neighbor.x, point.y - neighbor.y);
    if (distance < radius * .72) { ctx.strokeStyle = `hsla(${hue}, 90%, 63%, ${.08 + point.z * .16 + brainLevel * .1})`; ctx.lineWidth = .55 + brainLevel; ctx.beginPath(); ctx.moveTo(point.x, point.y); ctx.lineTo(neighbor.x, neighbor.y); ctx.stroke(); }
  }
  for (const point of points) { const dot = 1 + point.z * 1.4 + brainLevel * 2.5; ctx.fillStyle = `hsla(${hue + point.z * 25}, 100%, ${67 + point.z * 20}%, ${.34 + point.z * .55})`; ctx.beginPath(); ctx.arc(point.x, point.y, dot, 0, Math.PI * 2); ctx.fill(); }
  ctx.globalCompositeOperation = 'source-over';
  const glow = ctx.createRadialGradient(centerX, centerY, radius * .1, centerX, centerY, radius * 1.15);
  glow.addColorStop(0, `hsla(${hue}, 90%, 65%, ${.08 + brainLevel * .18})`); glow.addColorStop(1, 'transparent'); ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(centerX, centerY, radius * 1.2, 0, Math.PI * 2); ctx.fill();
}

updateBrainModel(modelSelect.value, modelSelect.options[modelSelect.selectedIndex]?.dataset.llm || '');
drawBrain();
...
function setStatus(state, text) {
  ...
  connectionStatus.className = 'talk-status';
  ...
  if (state === 'listening') setBrainState('listening');
  else if (state === 'thinking') setBrainState('thinking');
  else if (state === 'speaking') setBrainState('speaking');
  else if (state === 'disconnected' || state === 'connected' || state === 'connecting') setBrainState('idle');
}
...
function addTranscript(role, text) {
  ...
  const placeholder = transcript.querySelector('.transcript-empty, .italic');
...
async function connect() {
  ...
  try {
    ...
    startBrainVisualizer(localStream, 'local');
    ...
    pc.ontrack = (event) => {
      ...
      startBrainVisualizer(event.streams[0], 'remote');
...
function disconnect() {
  ...
  localBrainAnalyser = null;
  remoteBrainAnalyser = null;
  setBrainState('idle');
