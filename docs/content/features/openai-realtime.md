...
#### Talk page speaking indicator

In LocalAI's built-in **Talk** page, the model-presence orb reacts only to the
assistant's received WebRTC audio. Your microphone activity does not animate
it. While LocalAI is speaking, the orb expands and brightens; it returns to its
idle state when the spoken response ends.
